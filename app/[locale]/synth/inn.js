...
const t = useTranslations("SynthPage");
...
useEffect(()=>{
    ...
    <main className={styles.main}>
        <div className={[styles.description, 'center-block text-center m-2']} style={{lineHeight: '3rem'}}>
            <span>{ t("page_description") }<h5><a href="https://tonejs.github.io/" target='blank'>tone js</a></h5></span>
        </div>
        <div>
            ...
            <select name="presets" id="presets" onChangeCapture={(e)=>{console.log(e.target.value);renderSynth(localStorage.getItem('octave')||3,localStorage.getItem('keysLength')||12,e.target.value)}}>
                <option value="piano">{ t("presets_piano") }</option>
                <option value="drums">{ t("presets_drums") }</option>
            </select><br/>
            <span className={styles.description}>{ t("presets_hint") }</span>
        </div>
        <div className="center-block container">
            <h2 id='octaveValue' className='text-center'>{ t("leastoctave_label") }: { octave }</h2>
            <div className="row">
            <div className='col'>
                <button id='octave+' className='center-block' onClick={increaseOctave}>{ t("octave_label") }+</button>
            </div>
            <div className='col'>
                <button id='octave-' className='center-block' onClick={decreaseOctave}>{ t("octave_label") }-</button>
            </div>
            </div>
        </div>
        <div className="center-block container">
            <h2 id='keysValue' className='text-center'>{ t("keys_label") }: { keysLength }</h2>
            <div className="row">
            <div className='col'>
                <button id='key+' className='center-block' onClick={increaseKeysLength}>{ t("addkey_label") }</button>
            </div>
            <div className='col'>
                <button id='key-' className='center-block' onClick={decreaseKeysLength}>{ t("removekey_label") }</button>
            </div>
            </div>
        </div>
        <div id="sampler" style={{ border: "3px solid black", borderRadius: "15px", background: "color-mix(in srgb, var(--background) 90%, white)", margin: "1.5rlh 0", padding: "0.5rlh", width: "80vw" }}>
            ...
            <div id="samplerouter" style={{ display: "flex", justifyContent: "center", flexDirection: "column", alignItems: "center" }}>
                <div id="BPMTapper">
                    ...
                    <button style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "center", border: "3px solid var(--nav-background)", borderRadius: "10%", height: "6rlh", width: "6rlh", textAlign: "center", verticalAlign: "middle" }} onClick={(e)=>setBPMtext(e)}>
                        { t("tapper_label") }
                    </button>
                </div>
                <form onSubmitCapture={(e)=>{e.preventDefault();processSampler(e);renderSampler();return false;}}>
                    <input onChange={(e)=>{console.log(e)}} type='text' placeholder={ t("sample_name_placeholder") } style={{ margin: "0.25rlh auto" }} id="sampleName"/><br/>
                    <input onChange={(e)=>{console.log(e)}} type="text" placeholder={ t("sample_note_placeholder") } pattern='[ACDFG]{1}#?[01234567]{1}|[BE]{1}[01234567]{1}' onInvalidCapture={(e)=>{console.log(e.target.validity.valid);e.target.setCustomValidity(/[ACDFG]{1}#?[01234567]{1}|[BE]{1}[01234567]{1}/.test(e.target.value) ? "" : t("invalid_sample_note"))}} style={{ margin: "0.25rlh auto" }} id="noteName" required/><br/>
                    <input onChange={(e)=>{console.log(e)}} value={ t("sample_submit") } type="submit" style={{ display: "block", margin: "0.25rlh auto" }}/>
                </form>
            </div>
            <div id="samples">
